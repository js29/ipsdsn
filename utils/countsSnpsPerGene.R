...
option_list <- list(
  ...
  make_option(c("-w", "--window"), type="integer", default=500000,
              help="Size of the cis window in bp.", metavar="number"),
  make_option(c("-u", "--usefeaturecentre"), type="logical", default=FALSE,
              help="Define the window around a feature based on the center of the feature, rather than the start.")
)
opt <- parse_args(OptionParser(option_list=option_list))
if (is.null(opt$s) | is.null(opt$e)| is.null(opt$counts)){
...
if (!is.null(opt$usefeaturecentre) & opt$usefeaturecentre) {
  TSS_window = dplyr::group_by(exon_df, gene_id) %>% 
    summarise(chromosome_name = chromosome_name[1], start = ((exon_start + exon_end) / 2))
} else {
  TSS_window = dplyr::group_by(exon_df, gene_id) %>% 
...
#Add SNP counts into the original data.frame and write results to disk
