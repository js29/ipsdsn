...
ase_file = gzip.open(args.ASEcounts)
...
for line in sg_map_file:
	line = line.rstrip()
	fields = line.split("\t")
	sample_genotype_dict[fields[0]] = fields[1]

#Construct a sample name dict
ase_sample_dict = dict()
for i in range(0,len(ase_sample_names)):
	if (ase_sample_names[i] in sample_genotype_dict):
		genotype_id = sample_genotype_dict[ase_sample_names[i]]
...
with gzip.open(args.VCFfile) as vcf_file:
